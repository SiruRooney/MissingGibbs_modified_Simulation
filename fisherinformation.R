...
fisherinfo=function(predata,bape,ind.bape,rs.ind){
  ...
  mis.type=sapply(c(1:3),Bitmatrix)
    ...
    yxrs.T=cbind(y1T,x1T1,x1T2,r1T1,r1T2,s1T)
    ...
    D2y.k=t(apply(predatars.inpt[(comp.ind==TRUE),1:NROW(ind.bape[[1]])][,ind.bape[[1]]==1],1,function(x){x%o%x}))*(-p.y*(1-p.y))
    D2y.k=colSums(D2y.k)
    D2y=matrix(D2y.k,sum(ind.bape[[1]]),sum(ind.bape[[1]]))
    
    D2x1.k=t(apply(predatars.inpt[(comp.ind==TRUE),1:NROW(ind.bape[[2]][[1]])][,ind.bape[[2]][[1]]==1],1,function(x){x%o%x}))*(-p.x1*(1-p.x1))
    D2x1.k=colSums(D2x1.k)
    D2x1=matrix(D2x1.k,sum(ind.bape[[2]][[1]]),sum(ind.bape[[2]][[1]])) 
    
    D2x2.k=t(apply(predatars.inpt[(comp.ind==TRUE),1:NROW(ind.bape[[2]][[2]])][,ind.bape[[2]][[2]]==1],1,function(x){x%o%x}))*(-p.x2*(1-p.x2))
    D2x2.k=colSums(D2x2.k)
    D2x2=matrix(D2x2.k,sum(ind.bape[[2]][[2]]),sum(ind.bape[[2]][[2]])) 
    
    D2r1.k=t(apply(predatars.inpt[(comp.ind==TRUE),1:NROW(ind.bape[[3]][[1]])][,ind.bape[[3]][[1]]==1],1,function(x){x%o%x}))*(-p.r1*(1-p.r1))
    D2r1.k=colSums(D2r1.k)
    D2r1=matrix(D2r1.k,sum(ind.bape[[3]][[1]]),sum(ind.bape[[3]][[1]])) 
    
    D2r2.k=t(apply(predatars.inpt[(comp.ind==TRUE),1:NROW(ind.bape[[3]][[2]])][,ind.bape[[3]][[2]]==1],1,function(x){x%o%x}))*(-p.r2*(1-p.r2))
    ...
    for (i.mis in 1:NROW(predata[comp.ind==FALSE,])){#NROW(predata[comp.ind==FALSE,])
      ...
      yxrs.F=cbind(y1F,x1F1,x1F2,r1F1,r1F2,s1F)
      ...
      D2x1.k=t(apply(predamis.k[,1:NROW(ind.bape[[2]][[1]])][,ind.bape[[2]][[1]]==1],1,function(x){x%o%x}))*(-p.x1)*(1-p.x1)*con.P
      D2x1.k=colSums(D2x1.k)
      D2x1=D2x1+matrix(D2x1.k,sum(ind.bape[[2]][[1]]),sum(ind.bape[[2]][[1]]))
      D2x2.k=t(apply(predamis.k[,1:NROW(ind.bape[[2]][[2]])][,ind.bape[[2]][[2]]==1],1,function(x){x%o%x}))*(-p.x2)*(1-p.x2)*con.P
      D2x2.k=colSums(D2x2.k)
      D2x2=D2x2+matrix(D2x2.k,sum(ind.bape[[2]][[2]]),sum(ind.bape[[2]][[2]]))
      D2r1.k=t(apply(predamis.k[,1:NROW(ind.bape[[3]][[1]])][,ind.bape[[3]][[1]]==1],1,function(x){x%o%x}))*(-p.r1)*(1-p.r1)*con.P
      D2r1.k=colSums(D2r1.k)
      D2r1=D2r1+matrix(D2r1.k,sum(ind.bape[[3]][[1]]),sum(ind.bape[[3]][[1]]))
      D2r2.k=t(apply(predamis.k[,1:NROW(ind.bape[[3]][[2]])][,ind.bape[[3]][[2]]==1],1,function(x){x%o%x}))*(-p.r2)*(1-p.r2)*con.P
      D2r2.k=colSums(D2r2.k)
